...
def plot_belief(belief, step):
    ...
    #     s=22
    labels = belief
    states = [str(state) for state in labels]
    probabilities = [belief[state] for state in labels]

    plt.figure(figsize=(6, 4))
    plt.bar(states, probabilities, color='red')
    plt.xlabel('States')
    plt.ylabel('Probability')
    plt.title(f'Belief State at Step {step}')
    plt.ylim(0, 1)
    plt.show()
...
def test_planner(tiger_problem, planner, nsteps=3, debug_tree=False):
    ...
    for i in range(nsteps):
        ...
        if isinstance(tiger_problem.agent.cur_belief, pomdp_py.Histogram):
            ...
            plot_belief(tiger_problem.agent.cur_belief, i + 1)
...
def main():
    ...
    test_planner(tiger, vi, nsteps=4)
